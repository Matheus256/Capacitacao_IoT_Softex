...
static void mqtt_initialize(void) {/*Depending on your website or cloud there could be more parameters in mqtt_cfg.*/
    ...
    const esp_mqtt_client_config_t mqtt_cfg = {
      .username = "matheus_nascimento:2c938e",
...
void blink(void *pvParameter) {
    while(1) {
        int msg_sub_id = esp_mqtt_client_subscribe(client, "matheus_nascimento:8b1e21/attrs", 0);
...
void send_data(void *pvParameter){
  while(1){
    ...
    char str_data[50];
    sprintf(str_data,"{\"Temperatura\": %.1f }",Temperature);
    ...
    printf("Enviando -> '%s'\n",str_data);
    ///matheus_nascimento:2c938e/attrs
    int msg_pub_id = esp_mqtt_client_publish(client, "matheus_nascimento:2c938e/attrs", str_data, strlen(str_data), 1, 0);
...
void app_main() {	
    ...
    //int msg_sub_id = esp_mqtt_client_subscribe(client, "matheus_nascimento:8b1e21/attrs", 1);

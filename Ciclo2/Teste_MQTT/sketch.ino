...
const char* mqttUser = "user_name";  // Apenas o usuário, sem senha
const char* mqttTopic = "device_id/attrs";  // Substitua <device_id> pelo ID do dispositivo no Dojot
...
void reconnect() {
  while (!client.connected()) {
    ...
    // Conecta apenas com o usuário, sem senha
    if (client.connect("ESP32Client", "user_name:device_id", "")) {
...
void loop() {
  ...
  char payload[32];
  sprintf(payload, "{\"Valor\": %d}", value);

  Serial.print("Enviando payload: ");
  Serial.println(payload);

  if (client.publish("user_name:device_id/attrs", payload)) {
